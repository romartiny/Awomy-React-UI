...
export const HeaderLayout = () => {
  ...
  const burgerToggleNavigation = () => setIsOpenNavigation(!isOpenNavigation);
    ...
    <>
        ...
        <div className="items-center hidden lg:flex lg:gap-10 pl-16">
          <>
            {navigationLinks.map(({
                ...
                <Link to={href} key={_id}
                      className="relative -mx-3 -my-2 py-2 px-3 rounded-lg text-base transition-colors text-white"
                      onMouseEnter={() => setHoverIndex(_id)} onMouseLeave={() => setHoverIndex(null)}>
                  <AnimatePresence>
                    {
                        hoverIndex === _id && (
                            <motion.span className="rounded-lg bg-gray-400 absolute inset-0" layoutId="hoverBackgorund"
      ...
      <div className="items-center flex grap-6 cursor-pointer">
        <div className="z-50 flex items-center lg:order-2">
          ...
          <button type="button" onClick={() => burgerToggleNavigation()}
                  className="inline-flex items-center p-2 ml-1 text-sm text-gray-500 rounded-lg lg:hidden dark:text-gray-400 dark:hover:bg-gray-700 transition ease-in-out delay-70"
            ...
            isOpenNavigation && (
              <>
                <Popover className="lg:hidden">
                  <Popover.Overlay static as={motion.div} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                                   exit={{ opacity: 0 }} className="fixed z-20 inset-0 bg-gray-600/10 backdrop-blur"/>
                  <Popover.Panel static as={motion.div} initial={{
                    opacity: 0,
                    y: -32
                  }} animate={{
                    opacity: 1,
                    ...
                    transition: { duration: 0.2 }
                                 ...
                                 className="absolute inset-x-0 top-0 z-30 origin-top rounded-b-2xl bg-gray-800 px-6 pb-6 pt-28 shadow-2xl shadow-gray-900/20">
                    <div className="space-y-4">
                      {
                        navigationLinks.map(({
                          ...
                          <MobileNavigationLinks href={href} key={_id} icon={icon}
                                                 mobile={() => toggleOpenNavigation(false)}>{title}</MobileNavigationLinks>
                        ))
                      }
                    </div>
                    <div className="pt-6">
