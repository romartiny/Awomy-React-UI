      ...
      <Container id="account" aria-label="2" className="py-20 sm:py-32">
        <FrameMotion initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 130, damping: 25 }}>
          <div className=" mx-auto max-w-2xl sm:text-center">
            ...
            <p className="mt-2 text-lg text-gray-600">{consData.description}</p>
          </div>
        </FrameMotion>
        <FrameMotion initial={{ opacity: 0, y: -40 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 130, damping: 25 }}>
          <ul role="list" className="mx-auto mt-16 grid max-w-2xl grid-cols-1 gap-6 text-sm sm:mt-20 sm:grid-cols-2 md:gap-y-10 lg:max-w-none lg:grid-cols-3">
            {accountData.map((item) => (
                <li key={item.name} className="rounded-2xl border border-gray-200 hover:border-gray-200 p-8 group hover:bg-gray-200 duration-300 cursor-pointer">
